...
- CI fails when the JSON Schemas regenerated by `npm run check` differ from the committed files in `schemas/`. Previously a zod upgrade or a schema edit could change the published schemas without any check failing.
